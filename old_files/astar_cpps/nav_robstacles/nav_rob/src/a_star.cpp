...
{
    cout<<"I'm getting the map1"<<endl;
    width = msg->info.width;
    cout<<"width2"<<endl;
    height = msg->info.height;
    cout<<"I'm getting the map3"<<endl;
    resolution = msg->info.resolution;
    cout<<"I'm getting the map4"<<endl;
    center = msg->info.width / 2;
    init_map = new int*[msg->info.height];
    for (int i = 0; i < msg->info.height; i++)
        init_map[i] = new int[msg->info.width];
    cout<<"I'm getting the map5"<<endl;		
    for (int i = 0; i < msg->info.height; i++)
        for (int j = 0; j < msg->info.width; j++) {
            if (msg->data[i*msg->info.width + j] == 100) {
                init_map[j][i] = INF;
                IntrinsicCost(j, i);
    cout<<"I'm getting the map6"<<endl;
            }
        }
...
{
    cout<<"I'm getting the initial pose"<<endl;
...
void goalCallback(const move_base_msgs::MoveBaseGoal::ConstPtr& msg)  //const move_base_msgs::MoveBaseGoal::ConstPtr& msg
// subscribe final goal that published goal_publisher node and make class Path p and publish the path
{
    cout<<"I'm getting the goal"<<endl;
